...
switch($method) {
    case 'GET':
        ...
        if(isset($_GET["id"])) {
            //if id in GET, get specific User, else get all Users
    ...
    case 'POST':
        ...
        if($data->username == "" || $data->password == "") {
            $response = array("message" => "Please send User title, url, description and creation date");

            http_response_code(400);//error
        } else {
            if($User->addUser($data->username, $data->password)) {
                $response = array("message" => "Created");
                http_response_code(201); //Created
            } else {
                $response = array("message" => "Error adding User");
        ...
        case 'PUT':
            ...
            if(!isset($id)) {
                ...
                $response = array("message" => "No User ID is sent");
            //Om code är skickad   
            } else {
                $data = json_decode(file_get_contents("php://input"));
                //checks if any value is empty
                if($data->username == "" || $data->password == "") {
                    ...
                    $User->updateUser($data->username, $data->password, $id);
                    http_response_code(200);
                    $response = array("message" => "Post with User ID: $id is updated");
                }
    ...
    case 'DELETE':
        if(!isset($id)) {
            ...
            $User->deleteUser($id);
            http_response_code(200);
            $response = array("message" => "Post with User ID $id is deleted");
        }
        break;
